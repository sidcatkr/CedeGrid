...
The GitHub Packages mirror published all six scoped packages and passed exact
archive download checks plus fresh SDK/native alias installation. The SDK and
Windows package visibility were verified public. The mirror preserves original
payload bytes; it does not include the source follow-up fixes.

The published macOS arm64 coordinator also passed first-page measurements at
100,000 and 1,000,000 tasks, with 30 requests for each unfiltered/job-scoped query:
maximum per-scope p95 was 19.61 ms and maximum additional peak RSS was 1,638,400
bytes. These measurements used the original wheel. The database seed and EXPLAIN
helper came from an earlier candidate, so this is direct latency/RSS evidence,
not a complete final-candidate query-plan qualification.

Follow-up source CI passed all 15 Node/Python SDK combinations and the Rust 1.88
check. Both Linux architectures passed the four distributed scenarios that had
failed for the published candidate. CI also identified fixture log placement and
an obsolete expectation that read-only state reopening must acquire a write lock;
three focused corrections passed locally while retaining crash-recovery and
OS-lock assertions. Unix execution fixtures were scoped away from unsupported
Windows execution. These are follow-up source results, not replacements for the
published binary results.

The macOS Intel CI run still reported CPU observation above a single-core
allocation in `full_single_cpu_does_not_yield_from_counter_rounding`. This remains
unresolved. Complete platform, GPU, artifact-load, and final-artifact qualification
is not claimed.
